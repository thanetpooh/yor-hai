    ...
    <div className="relative flex min-h-screen flex-col bg-skin">
      ...
      <main className="relative z-10 mx-auto w-full max-w-5xl flex-1 px-10 pb-10 pt-14 max-sm:px-6 max-sm:pt-9">
        ...
        <section className="mb-9">
          <div className="mb-3 flex items-center gap-2.5 font-mono text-xs uppercase tracking-widest text-ink-muted">
            <span className="h-px w-9 bg-rule-strong" />
            ย่อลิงก์ + สร้าง QR
          </div>
          <h1 className="mb-3 font-serif text-4xl font-normal leading-tight tracking-tight sm:text-6xl">
            วางลิงก์ยาวๆ ไว้ตรงนี้
            <br />
            รับ <em className="italic text-accent-ink">ลิงก์สั้น</em> และ{" "}
            <em className="italic text-accent-ink">QR code</em> ทันที
          </h1>
          <p className="max-w-xl text-lg text-ink-soft">
            ช่องเดียว ได้สองอย่าง — ย่อผ่าน server ของตัวเอง และสร้าง QR
            โหลดได้เป็น PNG หรือ SVG
          </p>
        </section>
      ...
      <footer className="relative z-10 mt-12 flex flex-wrap items-center justify-between gap-3 border-t border-rule px-10 py-7 font-mono text-xs text-ink-muted max-sm:px-6">
        <span>© 2026 · Snip &amp; Scan</span>
        <a
